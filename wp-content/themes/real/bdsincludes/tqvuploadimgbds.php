...
@unlink("images/newsbds/".$c[tqvbdsimages_url1]);
  ...
  function edit() {
  	 ...
  	 $imgid = intval( $_GET["imgid"] );
  	 $sqllefff=@mysql_query("select * from tqv_bdsnewsbds where tqvnewsbds_id ='$imgid'");
    $rowlefff=@mysql_fetch_assoc($sqllefff);
    $tqvnewsbds_tieude=$rowlefff['tqvnewsbds_tieude'];
  	 $sqlle=@mysql_query("select * from tqv_bdsnewsimagesbds where tqvbdsimages_id ='$idedit'");
     $rowle=@mysql_fetch_assoc($sqlle); 
...
elseif(!getimagesize($_FILES['file_upload']['tmp_name'])) { 
  	...
  	$chuthich=$_POST['chuthich'];
   $member_id = addslashes($_SESSION['member_id']);
   $q3 = "update  tqv_bdsnewsimagesbds set tqvnewsbds_id='$imgid',tqvbdsimages_url1='$color_1', tqvbdsimages_note='$chuthich',tqvadmin_id='$matv'
