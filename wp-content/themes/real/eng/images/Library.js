...
{
	if(field.value=='T? khóa tìm ki?m'){ field.value = ''; }
}

function SearchOnBlur(field)
{
	if(field.value==''){ field.value='T? khóa tìm ki?m';}
}

function NameOnFocus(field)
{
	if(field.value=='H? tên'){ field.value = ''; field.className = 'adword-textbox2'}
}

function NameOnBlur(field)
{
	if(field.value==''){ field.value='H? tên'; field.className = 'adword-textbox'}
}
...
{
	if(field.value=='Tiêu d?'){ field.value = ''; field.className = 'adword-textbox2'}
}

function TitleOnBlur(field)
{
	if(field.value==''){ field.value='Tiêu d?'; field.className = 'adword-textbox'}
}

function ValidateCodeOnFocus(field)
{
	if(field.value=='Mã xác nh?n'){ field.value = ''; field.className = 'adword-textbox2'}
}

function ValidateCodeOnBlur(field)
{
	if(field.value==''){ field.value='Mã xác nh?n'; field.className = 'adword-textbox'}
}
...
{	
	...
	document.writeln('	<div class="midS">');
	document.writeln('		<input type="text" class="txtSearch" name="s" id="s" onblur="SearchOnBlur(this)" onkeyup="initTyper(this)" onkeydown="" onfocus="SearchOnFocus(this)" value="T? khóa tìm ki?m" />');
	document.writeln('	</div>');
	document.writeln('	<div class="rightS"></div>');
	document.writeln('	<div class="goS">');
	document.writeln('		<input type="button" class="txtgoS" onClick="return CheckValue(document.Search.s);" />');
	document.writeln('	</div>');
...
function CheckValue(o){
	if(Trim(o.value)=='T? khóa tìm ki?m'||Trim(o.value)==''){
		o.value = '';
		o.focus();
		return false;
	}
...
function ShowTopBanner()
{
	...
	{		
		...
		document.writeln(ShowFlashObject('AdTopBanner', 'lhqc_vne_728x90_ebank.swf', 728, 90));
		document.writeln('</div>');	
		return;
	}
}
...
function ShowRightLogo(vType)
{
	...
	if (vType==1){
		if (RefVal8.length>0)
		{
			...
			var alRightLogo1 = new adlistshow(RefVal8,'RightLogo1',0,1,0,147,51);
			document.writeln('</div>');
		}
		else
		{			
		...
		{
			document.writeln('<div class="fl">');
			var alRightLogo2 = new adlistshow(RefVal9,'RightLogo2',0,10,0,147,157);
			document.writeln('</div>');
		}
		else
		{				
...
function ShowLargeLogo(vType)
{
	...
	if (vType==1){
		...
		{
			document.writeln('<div class="box-item">');					
			var alLargeLogo2 = new adlistshow(RefVal5,'LargeLogo2',0,10,0,300,120,true,1);
			document.writeln('</div>');
			return;
		}
		else
...
function ShowBigLogo()
{
	document.writeln('<div class="fl">');
	if(PAGE_FOLDER==600){
		var alBigLogo1 = new adlistshow(RefVal6,'BigLogo1',0,1,0,218,250);
	}
	else{
...
function ShowBigLogo2()
{
	document.writeln('<div class="fl">');
	if(PAGE_FOLDER==600){
		var alBigLogo1 = new adlistshow(RefVal18,'BigLogo2',0,1,0,218,250);
	}
	else{
...
{	
	...
	if (typeof(vAd) == 'undefined') vAd = 0;
	if(PAGE_FOLDER==600){
		var alRightExpand = new adlistshow(RefVal17,'RightExpandLogo',vAd,10,0,218,150,false,3);
	}
	else
	{
...
function ShowSpecialBanner(vAd)
{
	...
	document.writeln('<div class="fl">');	
	if(PAGE_FOLDER==600){
		var alBasicLogo = new adlistshow(RefVal19,'SpecialLogo',vAd,1,0,218,600,true);
	}
	else{
...
function ShowLogoRight(vAd) {	
	if (RefVal6.length+RefVal7.length+RefVal17.length+RefVal18.length+RefVal19.length==0){		
		document.writeln('<div class="fl">');		
		if(PAGE_FOLDER != 600) {			
			document.writeln(ShowFlashObject('AdLogoRight', 'lhqc_vne_147x157_ebank.swf', 147, 157));
		}
		else {
			//document.writeln(ShowFlashObject('AdLogoRight', 'lhqc_vne_180x150.swf', 180, 150));
			document.writeln('<img src="/Images/AdWord/lhqc_218x130.jpg" alt"Lien he quang cao" />');
		}		
		document.writeln('</div>');
		return;
	}	
...
{	
	...
	switch(vAd){
		...
		case 3:
			...
			{
				document.writeln('<div>');		
				var alTopBanner = new adlistshow(RefVal26,'FixFloatBottomRightBanner',0,14,0,120,300,true,3);		
				document.writeln('</div>');
			}
			break;
	}	
    ...
    this.move = function() {
        try {
            ...
            if (l >= 603) {
                ...
                document.getElementById(arrdiv[ts_i]).style.left = "-603px";
...
function createHTML(arItemTS) {
	...
	if(iTSCount >= 3) {
		for (var i = 3; i <= iTSCount; i++) {
			sOtherItems = sOtherItems.concat('<p class="Top-News"><a href="' + arItemTS[i][4] + '" title="' + arItemTS[i][2] + '">' + arItemTS[i][2] + '</a></p>');			
		}
	}
	
    for (var i = 0; i <= iTSCount, i < 3; i++) {
        arrdiv[i] = "div" + i;
        arrdiv2[i] = "div_" + i;
    }
    ...
    for (var i = 0; i <= iTSCount, i < 3; i++) {		
		if (i == 0) {
			sHTML = sHTML.concat('<div id="div' + i + '" style="position:absolute;width:485px;height:180px;overflow:hidden;">');			
			if(arItemTS[i][3] != ''){
				sHTML = sHTML.concat('	<a class="aTopNews" href="' + arItemTS[i][4] + '" title="' + arItemTS[i][2] + '"><img src="' + arItemTS[i][1] + '" alt="' + arItemTS[i][2] + '" width="220" height="160" /></a>');
			}
			sHTML = sHTML.concat('<div class="ABTop">');			
			sHTML = sHTML.concat('	<div class="ATop">');			
			sHTML = sHTML.concat('		<p class="topTitle">');
			sHTML = sHTML.concat('			<a href="' + arItemTS[i][4] + '" onfocus="this.blur();" title="' + arItemTS[i][2] + '">' + arItemTS[i][2] + '</a>');
			sHTML = sHTML.concat('		</p>');
			sHTML = sHTML.concat('		<p class="topLead">' + arItemTS[i][3] + '</p>');
			sHTML = sHTML.concat('	</div>');
			sHTML = sHTML.concat('	<div class="BTop">');
			sHTML = sHTML.concat(TopStoryNav());						
			sHTML = sHTML.concat('		<div class="Top-Other">');
			sHTML = sHTML.concat(sOtherItems);
			sHTML = sHTML.concat('		</div>');
			sHTML = sHTML.concat('	</div>');
			sHTML = sHTML.concat('</div>');
			sHTML = sHTML.concat('</div>');			
		} else {
			sHTML = sHTML.concat('<div id="div' + i + '" style="position:absolute;width:485px;height:180px;overflow:hidden;left:600px;">');
			if(arItemTS[i][3] != ''){
				sHTML = sHTML.concat('	<a class="aTopNews" href="' + arItemTS[i][4] + '" title="' + arItemTS[i][2] + '"><img src="' + arItemTS[i][1] + '" alt="' + arItemTS[i][2] + '" width="220" height="160" /></a>');
			}
			sHTML = sHTML.concat('<div class="ABTop">');			
    ...
    slideShowObj1.b = setTimeout("slideShowObj1.a=window.setInterval('slideShowObj1.move()',20)", 5000);
    slideShowObj1.refobj(slideShowObj1);
}

function TopStoryNav(){
	var sHTML = '';
	sHTML=sHTML.concat('	<p class="TopButton">');
	sHTML=sHTML.concat('		<a href="javascript:void(0);" onfocus="this.blur();" onclick="obj.callMove1();"><img alt="Next" src="images/topRow-blue.gif" class="topRow-Red"/></a>');
	sHTML=sHTML.concat('		<a href="javascript:void(0);" onfocus="this.blur();" onclick="obj.callMove2();"><img alt="Back" src="images/topRow-Red.gif" class="topRow-Blue"/></a>');
	sHTML=sHTML.concat('	</p>');
	return sHTML;
}
