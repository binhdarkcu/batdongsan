...
require_once("../bdssecuritypt/bdssecurity.php");
  ...
  if(isset($submit) && $submit == 'Update')
  {
    ...
    $q3 = "insert into tqv_bdschapter set
    chapterkt_name=\"$alt\",
    chapterkt_order = \"$thutu\",chapterkt_status='$tinhtrang'";
	$r3 = @mysql_query($q3) or die(mysql_error());
	echo "
	<div align=\"center\">
            <meta http-equiv=\"REFRESH\" content=\"3; url=tqvdemucbds.php\"><br><br><br><br><br><br><br><br><br>
  ...
  <tr>
	...
	<legend>Please stand by ...
      ...
      <br>
      <br>(<a href=\"tqvdemucbds.php\">Click v&#224;o &#273;&#226;y n&#7871;u k&#244; mu&#7889;n &#273;&#7907;i l&#226;u</a>)</p>
</fieldset></td>
  </tr>
</table></div><br><br>
</td></tr></table><br>";
...
if($_FILES['file_upload']['name'] == '') { 
	...
	$thutu = intval($_POST["thutu"]);
    $tinhtrang = intval($_POST["tinhtrang"]);
	$q3 = "update tqv_bdschapter set
	chapterkt_name=\"$name\",
  	chapterkt_order= \"$thutu\",
    chapterkt_status  = \"$tinhtrang\"
	";
	$q3 .= "
	where chapterkt_id=\"$id\"
	";
	$r3 = @mysql_query($q3) or die(mysql_error());
  echo "
	<div align=\"center\">
...
<script language='javascript' type='text/javascript'>
    ...
    {
    	temp = prompt( "", "'tqvdemucbds.php?" + pid );
    	return false;
    }
...
<?php
$n=0;
$sql=@mysql_query("SELECT * FROM `tqv_bdschapter` ORDER BY chapterkt_order asc") 
                                or die(mysql_error());
                                $line=0;
			                    while($row=@mysql_fetch_array($sql)) {
			                 	$line++;
			                 	$name=$row["chapterkt_name"];
                                $id =$row["chapterkt_id"]; 
                                $thutu = $row["chapterkt_order"]; 
                                $status=$row["chapterkt_status"];	
 echo"
...
<TD><center>
<a class='Market' href='tqvdemucbds.php?dialoose=Edit&id=$id' ><IMG SRC='images/edit.gif' alt='Sửa đề mục' height='16' widht='16' border='0' ></a>
</center></TD>
...
<TD><center><a href=\"javascript:baoloi('tqvdemucbds.php?dialoose=Delete&id=$id&file=$images')\"><IMG SRC='images/remove.gif' alt='Thư mục thường' border='0'></a></center></TD>
</TR>";
...
function Edit() {
$id = intval($_GET[id]);
$a = "select * from tqv_bdschapter where chapterkt_id = \"$id\"";
...
function Deletepost(){
	if(confirm('Bạn có chắc chắn muốn xóa không vậy?')){
		location.href='tqvdemucbds.php?dialoose=Delete&id=<? echo $c['chapterkt_id']; ?>';
	}
}
</SCRIPT>
<script src="images/date-picker.js"></script>
...
<table width="550" border="0" cellpadding="0" cellspacing="0">
    ...
    <tr> 
      ...
      <td><input onkeyup=initTyper(this); size=61 name="alt" value="<? echo $c['chapterkt_name']; ?>" type="text">
            </td>
    </tr>
  <tr> 
            <td valign="top"><B>Thứ tự:</B></td>
      <td><input size=61 name="thutu"  value="<? echo $c['chapterkt_order']; ?>" type="text">
            </td>
    </tr>
...
<SELECT NAME="tinhtrang" size=0>
	...
	<?php 
	$sqltd=@mysql_query("SELECT * FROM tqv_bdschapter WHERE chapterkt_id ='$id'");
	while ($rowtd=@mysql_fetch_assoc($sqltd)){
		$tinhtrang=$rowtd['chapterkt_status'];
		         if($tinhtrang==1){ ?>
                          <option value="<?php echo $rowtd['chapterkt_status']; ?>">Không xóa được</option>
                          <option value="0">Xóa được</option>
                 <?php } else { ?>
                 		  <option value="<?php echo $rowtd['chapterkt_status']; ?>">Xóa được</option>
                          <option value="1">Không xóa được</option>
                 	<?php }
    } 
    ?>	  
...
//delete the database record
		$q1 = "delete from tqv_bdschapter where chapterkt_id= '$id' ";
		@mysql_query($q1) or die(mysql_error());
echo "
	<div align=\"center\">
<meta http-equiv=\"REFRESH\" content=\"1; url=tqvdemucbds.php\"><br><br><br><br><br><br><br><br><br>
<table width=\"40%\"  border=\"0\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\">
  <tr>
    <td align=\"center\" valign=\"top\"><fieldset style=\"padding: 2; width:312px; height:59px\">
	<legend>Please stand by ...
