...
require_once("../bdssecuritypt/bdssecurity.php");
  ...
  if(isset($submit) && $submit == 'Update')
  {
    ...
    $q3 = "insert into tqv_bdslink set
    chapterkt_id='$demuc',linkkt_name=\"$alt\",
    linkkt_link= \"$lienket\",linkkt_status='$tinhtrang'";
	$r3 = @mysql_query($q3) or die(mysql_error());
	echo "
	<div align=\"center\">
            <meta http-equiv=\"REFRESH\" content=\"3; url=tqvlienketbds.php\"><br><br><br><br><br><br><br><br><br>
  ...
  <tr>
	...
	<legend>Please stand by ...
      ...
      <br>
      <br>(<a href=\"tqvlienketbds.php\">Click v&#224;o &#273;&#226;y n&#7871;u k&#244; mu&#7889;n &#273;&#7907;i l&#226;u</a>)</p>
</fieldset></td>
  </tr>
</table></div><br><br>
</td></tr></table><br>";
...
if($_FILES['file_upload']['name'] == '') { 
	...
	$q3 = "update tqv_bdslink set
	chapterkt_id=\"$demuc\",
  	linkkt_name= \"$tenlienket\",
    linkkt_link = \"$lienket\",
	";
	
	$q3 .= "
	linkkt_status=\"$tinhtrang\" where linkkt_id=\"$id\"
	";
	$r3 = @mysql_query($q3) or die(mysql_error());
  echo "
	<div align=\"center\">
...
<? $a="select * from tqv_bdschapter";
$result = @mysql_query($a) or die ("loi");
while ($row = @mysql_fetch_array($result))
{?><option value='<?=$row['chapterkt_id']?>'><?=$row['chapterkt_name']?></option>
<? }?> 
</select>
</td></tr>
<tr> 
...
<script language='javascript' type='text/javascript'>
    ...
    {
    	temp = prompt( "", "'tqvlienketbds.php?" + pid );
    	return false;
    }
...
<?php
$n=0;
$sql=@mysql_query("SELECT * FROM `tqv_bdslink` ORDER BY linkkt_id DESC") 
                                or die(mysql_error());
                                $line=0;
			                    while($row=@mysql_fetch_array($sql)) {
			                 	$line++;
			                 	$demuc_id=$row["chapterkt_id"];
			                 	$name=$row["linkkt_name"];
			                 	$lin=$row["linkkt_link"];
                                $id =$row["linkkt_id"]; 
                                $status=$row["linkkt_status"];	
 echo"
<TR onmouseover=\"this.bgColor='#CCCCCC'\" onmouseout=\"this.bgColor='#FFFFFF'\" bgColor=#ffffff>
<TD align=\"left\">&nbsp;&nbsp; $line
 </TD>
<TD align=\"left\">&nbsp;&nbsp;"; 
   $sqldmn=@mysql_query("select * from tqv_bdschapter where chapterkt_id='$demuc_id'"); $rowdm=@mysql_fetch_assoc($sqldmn); echo $rowdm["chapterkt_name"];
...
<TD><center>
<a class='Market' href='tqvlienketbds.php?dialoose=Edit&id=$id' ><IMG SRC='images/edit.gif' alt='Sửa đề mục' height='16' widht='16' border='0' ></a>
</center></TD>
...
<TD><center><a href=\"javascript:baoloi('tqvlienketbds.php?dialoose=Delete&id=$id&file=$images')\"><IMG border=0 SRC='images/remove.gif'></a></center></TD>
</TR>";
...
function Edit() {
$id = intval($_GET[id]);
$a = "select * from tqv_bdslink where linkkt_id = \"$id\"";
...
function Deletepost(){
	if(confirm('Bạn có chắc chắn muốn xóa hay không?')){
		location.href='tqvlienketbds.php?dialoose=Delete&id=<? echo $c['linkkt_id']; ?>';
	}
}
</SCRIPT>
<script src="images/date-picker.js"></script>
...
<table width="550" border="0" cellpadding="0" cellspacing="0">
   ...
   <tr> 
		...
		<?
	$sel_cat=@mysql_query("select * from tqv_bdschapter order by chapterkt_id");
	while($res=@mysql_fetch_array($sel_cat))
	{
?>
		<option value="<?php echo $res['chapterkt_id']; ?>" <?php if($c['chapterkt_id']==$res['chapterkt_id']){echo "selected";}?>><?echo $res['chapterkt_name'];?>
<?
	}
?>
				</select> </td>
  ...
  <tr> 
            <td valign="top"><B>Tên liên kết:</B></td>
      <td><input size=61 name="tenlienket"  value="<? echo $c['linkkt_name']; ?>" type="text">
            </td>
    </tr>
 <tr> 
            <td valign="top"><B>Liên kết:</B></td>
      <td><input size=61 name="lienket"  value="<? echo $c['linkkt_link']; ?>" type="text">
            </td>
    </tr>
...
<SELECT NAME="tinhtrang" size=0>
	...
	<?php 
	$sqltd=@mysql_query("SELECT * FROM tqv_bdslink WHERE linkkt_id='$id'");
	while ($rowtd=@mysql_fetch_assoc($sqltd)){
		$tinhtrang=$rowtd['linkkt_status'];
		         if($tinhtrang==1){ ?>
                          <option value="<?php echo $rowtd['linkkt_status']; ?>">Không xóa được</option>
                          <option value="0">Xóa được</option>
                 <?php } else { ?>
                 		  <option value="<?php echo $rowtd['linkkt_status']; ?>">Xóa được</option>
                          <option value="1">Không xóa được</option>
                 	<?php }
    } 
    ?>	  
...
$id =intval($_GET[id]);
		...
		$q1 = "delete from tqv_bdslink  where linkkt_id= '$id' ";
		@mysql_query($q1) or die(mysql_error());
echo "
	<div align=\"center\">
<meta http-equiv=\"REFRESH\" content=\"1; url=tqvlienketbds.php\"><br><br><br><br><br><br><br><br><br>
<table width=\"40%\"  border=\"0\" align=\"center\" cellpadding=\"0\" cellspacing=\"0\">
  <tr>
    <td align=\"center\" valign=\"top\"><fieldset style=\"padding: 2; width:312px; height:59px\">
	<legend>Please stand by ...
